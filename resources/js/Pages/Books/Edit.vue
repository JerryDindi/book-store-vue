<template>
    ...
    <Link :href="route('books.index')" class="text-indigo-600 hover:text-indigo-900 my-5 block">
        Back to All Books
    </Link>
            ...
            <div class="px-4 py-5 bg-white sm:p-6">
                <div class="grid grid-cols-6 gap-6">
                    ...
                    <div class="col-span-6">
                        ...
                        <input :class="{'border-red-500': form.errors.ISBN}" v-model="form.ISBN" type="text" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">

                        <div class="text-red-500 mt-2" v-if="form.errors.ISBN">{{ form.errors.ISBN }}</div>
                    </div>

                    <div class="col-span-6">
                        <label class="block text-sm font-medium text-gray-700">Author</label>
                        <input :class="{'border-red-500': form.errors.author_id}" v-model="form.author_id" type="text" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">

                        <div class="text-red-500 mt-2" v-if="form.errors.author_id">{{ form.errors.author_id }}</div>
                    </div>
...
export default {
    ...
    setup(props) {
        const form = useForm({
           ...
           salutation: props.book.ISBN,
           description: props.book.author_id,
        });
